...
let deserialize_graph_event (json : Yojson.Safe.t) : graph_event option =
  ...
  | `Assoc fields -> (
      ...
      | "NodeAdded" ->
          ...
          let name =
            try get_str "name" with _ -> Printf.sprintf "node_%d" id
          in
          let node_type = try get_str "node_type" with _ -> control_name in
          ...
          let props =
            ...
            | `Null -> None
            | `Assoc ps ->
                Some (List.map (fun (k, v) -> (k, property_value_of_json v)) ps)
          ...
          let ports =
            ...
            | `List lst ->
                List.map
                  (function `Int p -> p | _ -> failwith "Invalid port value")
                  lst
            | _ -> []
          in
      ...
      `Assoc
        [
          ...
          ( "properties",
            ...
            | Some props ->
                `Assoc
                  (List.map (fun (k, v) -> (k, property_value_to_json v)) props)
          );
          ("ports", `List (List.map (fun p -> `Int p) n.ports));
        ]
  | NodeRemoved nid ->
      `Assoc [ ("type", `String "NodeRemoved"); ("node_id", `Int nid) ]
